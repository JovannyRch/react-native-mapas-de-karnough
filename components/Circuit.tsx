...
export const CircuitComponent = React.memo(
  ({ variables, initGroups, isMaxiterm = false }: CircuitProps) => {
    const { width } = useWindowDimensions();

    const addNegations = (vars: string[]) => {
      let result = [];
      for (let v of vars) {
        result.push(v);
        result.push(v + "'");
    ...
    let middleValueY =
      middles.length == 2
        ? (mapsXY[middles[1]].y - mapsXY[middles[0]].y) / 2 +
          mapsXY[middles[0]].y
        : mapsXY[middles[0]].y;
    ...
    for (var key in mapsXY) {
      ...
      if (y > middleValueY) {
        ...
        htmlContent += `
            <div style="position: absolute; top: ${
              middleValueY + 5
            }px;left:${x}px;background-color: ${color}; width: 3px; height: ${
          y - middleValueY - 2
        }px;z-index: 3;" ></div>
            <div style="position: absolute; top: ${
              middleValueY + 5
            }px;left:${x}px;background-color: ${color}; width: ${
        ...
        //Hacia abajo
        htmlContent += `
            <div style="position: absolute; top: ${y}px;left:${x}px;background-color: ${color}; width: 3px; height: ${
          middleValueY - y - 5
        }px;z-index: 3;" ></div>
            <div style="position: absolute; top: ${
              middleValueY - 5
        ...
        //Ahi quedate
        htmlContent += `
            <div style="position: absolute; top: ${y}px;left:${x}px;background-color: ${color}; width: ${
          posXOr - x + height * 0.05
        }px; height: 3px;z-index: 3;" ></div>
            `;
      }
